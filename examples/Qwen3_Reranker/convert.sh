#!/usr/bin/env bash
# convert.sh — Qwen3-Reranker 一键转换(PC 端,x86_64,无需目标板)
#
#   1) export_llm.py   HF 模型 → .onnx / .config.pkl(task_type=2) / .tokenizer.gguf / .embed.bin
#   2) export_rknn.py  .onnx → .rknn / .weight  (w4a16 / grq / group32,seq_lens=[1,128])
#
# 与纯 LLM 的区别:输出是 query-document 相关性分数(0~1);
# export_rknn 用 do_quantization=False,故第 2 步不需要量化数据集。
# --quant 走 GRQ 预量化(需 CUDA),会用到 datasets/CMMLU/dataset.txt。
# 复用仓库根 .venv(full toolkit)。
set -euo pipefail
...
示例:
  ./convert.sh Qwen/Qwen3-Reranker-0.6B
  ./convert.sh --proxy http://localhost:7897 Qwen/Qwen3-Reranker-4B
EOF
}
...
DATASET="$REPO_ROOT/datasets/CMMLU/dataset.txt"        # export_rknn 用(生成物)
CALI_DATASET="$REPO_ROOT/datasets/CMMLU/dataset.json"  # export_llm GRQ 校准用
...
echo "════════════════════════════════════════════════════════"
echo " [Reranker] 模型: $MODEL   平台: $PLATFORM"
...
if [[ "$SKIP_ONNX" -eq 0 ]]; then
    echo ""; echo "[1/2] export_llm.py → ONNX / config(task_type=2) / tokenizer / embed ..."
    t0=$SECONDS
    "$PY" export_llm.py --model_path "$MODEL" --export_llm_path "$ONNX" \
        --cali_dataset "$CALI_DATASET" $QUANT $MODELSCOPE
...
if [[ "$SKIP_RKNN" -eq 0 ]]; then
    echo ""; echo "[2/2] export_rknn.py --platform $PLATFORM → .rknn / .weight (grq w4a16,do_quantization=False) ..."
    t0=$SECONDS
